...
2011:Aug:20:PM:06:43
	...
	+	messy code, so I may need to plan carefully 
...
struct ZonePattern
	...
	//data
		...
		bool fill_02_04:	6;
		char name[13];
		TradeItem_UnitDef_Array tradePerUnit;		//there are tw*tl*amtfloors units of trade, this defines item quantities per unit.
		std::vector<BITMAP*> sprites;
		std::vector<u8>	occubytes;
	//accessors
		inline bool IsPath()	{	return ispath;	}
		inline void SetIsPathState(bool state)	{	ispath=state;	}
	//methods
		ZonePattern();
