...
u16 IsoTerrain::maxht()
	{
	u16 M=ht(0,0);
	for(int x=0;x<mapwidth;x++)for(int y=0;y<maplength;y++)
		{	M=max(M,ht(x,y));	}
	return M;
	}
...
void IsoTerrain::DendriteRidges(int slopeht,int slopebase)
	{
	//create dendrite:
		Dendrite*d=new Dendrite(mapwidth,maplength,10);
		DendriteMapHeights(d);
	//fill dendrite map:
	...
	#if 0
		//Draw dendrite map:
			DendriteDrawer*dd=new DendriteDrawer(d,3);
			dd->Draw(0,0);
			delete dd;
			rest(1000);
	#endif
	//trace paths and lower land by the amount of visits for each point:
		d->TracePaths((mapwidth+maplength)*32);
		int curht;
		const int visitht=1;
		int hivisits=1;
		formap(0,0,mapwidth,maplength)
			{
			hivisits=max(hivisits,d->pt(x,y)->visittally);
			}
		cout<<"the highest visit tally on map is :"<<hivisits<<endl;

		//follow a fixed number of randome seeds, and raise a pyramid at each point encountered
			LatticePoint*lp;
			int amtseeds=d->getw()/16;
			for(int seedi=0;seedi<amtseeds;seedi++)
				{
				int x,y;
				bool done=false;
				x=rand()%d->getw();	y=rand()%d->getl();
				lp=d->pt(x,y);
				int pathlen=0;
				const int pathlen_max=50;
				while(!done)
					{
					if(lp==0)
						done=true;
					else if (lp==d->GetExitPoint())
						done=true;
					else if(pathlen>=pathlen_max)
						done=true;
					else
						{
						int rx,ry;
						rx=lp->x/d->getspacing();
						ry=lp->y/d->getspacing();
						RaisePyramid(rx,ry,4,1,3);
						lp=lp->next;
						}
					}
				////Smooth(x,y,0);
